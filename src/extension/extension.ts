...
const OUTLINE_VISIBILITY_STATE_KEY = 'markdownEasyVisualEditor.outlineVisible';
...
class MarkdownEasyVisualEditorProvider implements vscode.CustomTextEditorProvider {
        ...
        try {
            switch (message.type) {
                ...
                case 'setOutlineVisible':
                    await this.context.globalState.update(OUTLINE_VISIBILITY_STATE_KEY, message.visible);
                    this.broadcastSettings();
                    return;
                ...
                case 'exportPdf': {
                    ...
                    if (target) {
                        ...
                        this.post(panel, { type: 'operationFailed', requestId: message.requestId, message: this.getMessages().host.saveCanceled });
                    }
                    return;
                }
                case 'exportHtml': {
                ...
                case 'renderPdfPreview': {
                    ...
                    const previewPromise = (async () => {
                        ...
                        try {
                            ...
                            if (this.pdfPreviewGenerations.get(panel) !== generation || !isPanelActive()) return;
                            this.post(panel, { type: 'pdfPreviewReady', requestId: message.requestId, pdfBase64: pdf.toString('base64') });
                        } catch (error) {
            ...
            if (message.type === 'localChanges') {
                ...
                if (this.activeOperationKeysByDocument.get(documentKey) === operationKey) this.activeOperationKeysByDocument.delete(documentKey);
    ...
    private async queueWebviewEdit(
        ...
        const current = previous.catch(() => undefined).then(() => this.applyWebviewEdit(document, panel, message));
    ...
    private async queueHistoryCommand(
        ...
        const current = previous.then(async () => {
            ...
            if (registeredClientId && registeredClientId !== message.clientId) throw new Error(this.getMessages().host.clientIdMismatch);
    ...
    private async applyWebviewEdit(
        ...
        const registeredClientId = this.panelClientIds.get(panel);
        if (registeredClientId && registeredClientId !== message.clientId) throw new Error(this.getMessages().host.clientIdMismatch);
        const previousApplication = (this.changeHistory.get(key) ?? []).find((entry) => entry.clientId === message.clientId && entry.opId === message.opId);
    ...
    private onDocumentChanged(event: vscode.TextDocumentChangeEvent): void {
        ...
        const matchesActiveOperation = Boolean(
            active && baseVersion === active.appliedBaseVersion && nextCanonicalText === active.expectedText
        );
        ...
        if (active) {
            ...
            if (!matchesActiveOperation) {
                this.sendResync(active.panel, event.document, active.clientId, active.opId, 'Webview差分の適用中に別の文書変更を検出したため再同期します。');
            }
        }
    }
    ...
    private wasOperationApplied(key: string, clientId: string, opId: string): boolean {
        return (this.changeHistory.get(key) ?? []).some((entry) => entry.clientId === clientId && entry.opId === opId);
    }
    ...
    private async saveImages(
        ...
        for (const image of images) {
            ...
            if (bytes.byteLength > maxBytes) throw new Error(messages.app.errors.imageSize(settings.maxPasteSizeMb));
    ...
    private getSettings(document?: vscode.TextDocument): WebviewSettings {
        ...
        return {
            ...
            outlineVisible: this.context.globalState.get<boolean>(OUTLINE_VISIBILITY_STATE_KEY, true),
    ...
    private broadcastSettings(): void {
        for (const [key, panels] of this.panels) {
            ...
            for (const panel of panels) {
                this.post(panel, { type: 'settingsChanged', settings });
            }
        }
    }
...
function resolveLocalResourceUri(documentUri: vscode.Uri, source: string): vscode.Uri | undefined {
    ...
    if (/^\//.test(clean) && !/^\/\//.test(clean) && documentUri.scheme === 'file') return vscode.Uri.file(clean);
