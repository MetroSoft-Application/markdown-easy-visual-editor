...
export function renderMarkdown(markdown: string, options: RenderOptions): string {
  ...
  renderer.image = ({ href, title, text }) => {
    ...
    return `<img src="${escapeAttribute(href)}" data-original-src="${escapeAttribute(href)}" data-mve-image-index="${currentImageIndex}" data-mve-image-kind="markdown" data-mve-image-align="left" data-mve-resizable="${resizable ? 'true' : 'false'}" data-mve-can-reset="false" alt="${escapeAttribute(text)}"${titleAttribute}>`;
  };
  ...
  const html = tokens.map((token, index) => {
    ...
    return `<div class="markdown-source-block" data-source-from="${range.from}" data-source-to="${range.to}">${rendered}</div>`;
  }).join('') + renderFootnoteSection(footnoteDefinitions, messages);
  // 警告ブロックを専用要素へ変換してから、DOMPurifyで生成HTML全体を無害化する。
  const alerts = renderAlerts(html, messages);
  return DOMPurify.sanitize(alerts, {
    ADD_ATTR: [
      'target',
      'data-original-src',
      'data-mve-link',
